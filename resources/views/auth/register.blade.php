...
@section('titulo')
    Registrate en la app de Muebleria
@endsection
        ...
        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
            <form action="{{route('crear-cuenta')}}" method="POST" novalidate>
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="username" id="username" placeholder="Tu nombre de usuario" class="border p-3 w-full rounded-lg @error('username') border-red-500 
                    @enderror" value="{{ old('username') }}">
                    @error('username')
                        <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">{{ str_replace('username', 'nombre de usuario', $message) }}</p>
                    @enderror
                </div>
